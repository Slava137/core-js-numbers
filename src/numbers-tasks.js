...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vectorX = x1 ** 2 + y1 ** 2;
  const vectorY = x2 ** 2 + y2 ** 2;

  return Math.acos((x1 * x2 + y1 * y2) / Math.sqrt(vectorX * vectorY));
}
...
function getSumOfDigits(num) {
  const strNum = `${num}`;
  const arrOfStrNum = strNum.split('');
  const arrOfNumb = arrOfStrNum.map((item) => +item);

  return arrOfNumb.reduce((sum, prev) => sum + prev);
}
